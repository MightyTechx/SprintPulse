...
export type DocType = 'pdf' | 'xlsx' | 'svg' | 'pptx';
...
const ISSUES: Array<{ title: string; status: IncidentStatus; hours: number }> = [
  ...
  { title: 'Sprint board pagination jumps on filter', status: 'In Review', hours: 4.25 },
  { title: 'Webhook payload 502 from staging', status: 'Open', hours: 1.5 },
  { title: 'Null pointer in notification dispatcher', status: 'Testing', hours: 8 },
  ...
  { title: 'CSV export truncates long issue titles', status: 'In Review', hours: 2 },
  { title: 'Date picker locale not honouring IST', status: 'Open', hours: 0.5 },
  { title: 'Permission check bypass for guest viewers', status: 'Testing', hours: 5.5 },
...
export type AgileReportType = 'Daily Status Report' | 'Weekly Status Report' | 'Monthly Status Report';
...
const buildSprintLabel = (
  ...
  switch (reportType) {
    ...
    case 'Weekly Status Report': {
      // ISO week number (manual, no extra plugin required)
...
export const getAgileReportRows = (
  ...
  // safeFrom and safeTo are guaranteed non-null below (caller passes dates for active report)
  const safeFrom = (fromDate ?? toDate) as Dayjs;
  const safeTo = (toDate ?? fromDate ?? safeFrom) as Dayjs;

  return teams.map((team, idx) => {
    // Deterministic mock numbers based on team index + date seed
    const seed = Math.floor(safeFrom.valueOf() / 86_400_000) + idx * 7;
    const totalStoryPoints = 30 + ((seed * 13) % 41); // 30 – 70
    const totalCompletedStories = 4 + ((seed * 11) % 9); // 4 – 12
